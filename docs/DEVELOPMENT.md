# Geofencing Android — 개발 문서

> 골프카트 지오펜싱/모니터링 Android 앱. Site → Sector → Cart 계층으로 카트의 위반(Violation)·연결끊김(Disconnect) 상태를 지도와 리스트로 모니터링한다.
> 이 문서는 Confluence 게시용이며, 코드 기준 최신 상태를 반영한다. (기준 커밋: 아래 1번 참고)
...
| 현재 versionName / versionCode | `1.0` / `1` (`app/build.gradle.kts`) |
| 배포 APK 파일명 | `geofence_v1.2.apk` (Release 서명) |
| Git branch | `fix/status-row-cart-primary` |
| Git commit | `bffbfae` |
| Git tag | 없음 |
| 관련 PR | #7 (`fix/status-row-cart-primary` → `main`) |
...
- **Supabase** 프로젝트(임시 백엔드) 접근 정보. 초기 스키마/목데이터는 `supabase/*.sql` 참고.
- 인터넷 연결(권한: `INTERNET`).
...
└── ui/
    ...
    ├── home/                      # HomeScreen — 탭(WholeSector/Sector/Cart) 컨테이너 + 크롬
    ├── wholesector/               # WholeSectorPage + ViewModel + 샘플데이터 (사이트 전체 요약)
    ├── sector/                    # SectorPage + ViewModel + 샘플데이터 (섹터 상세)
    ├── cart/                      # CartPage + ViewModel + 샘플데이터 (카트 상세/지도)
    ├── map/                       # 지도: LiveGeofenceMap, GeofenceMapOverlay, 히트맵, 스냅샷, 검색
    ├── mock/                      # DashboardRepository(인터페이스) + SupabaseDashboardRepository(현행 구현) + MockSectors
...
| `HomeScreen` | 상단바 + 섹터 탭(`SectorTabRow`) + 페이지 전환(WholeSector/Sector/Cart)을 담는 컨테이너. |
| `WholeSectorViewModel` / `SectorViewModel` / `CartViewModel` | 각 페이지의 `@HiltViewModel`. `DashboardRepository`를 관찰해 `StateFlow<LoadState<...>>` 노출. |
| `DashboardRepository` | 대시보드 데이터 계약(`observeSectors/observeSector/observeCart/refresh`). |
| `SupabaseDashboardRepository` | **현행 구현.** Supabase(PostgREST)에서 섹터/카트/이벤트를 읽어 `LoadState`로 흘려보냄. `geofence_status`("violating"/"disconnected"/그외)를 UI 상태로 매핑. |
...
Supabase (PostgREST, 임시 BE)
        ...
        ▼
SupabaseDashboardRepository ──(LoadState<T>)──▶ WholeSector/Sector/Cart ViewModel ──(StateFlow)──▶ *Page (Compose)
        │
...
- **ViewModel 사용 방식**: 각 페이지가 `hiltViewModel()`로 전용 ViewModel을 주입받고, ViewModel은 Repository의 `Flow`를 `map`해 `stateIn(viewModelScope, WhileSubscribed(5s), Loading)`으로 화면 상태를 만든다. UI는 `collectAsState()`로 구독. 재시도는 `retry()` → `repository.refresh()`.
- **Repository 구조**: 계약(interface)과 구현(Impl) 분리, Hilt `@Binds`로 연결. **두 계층**이 공존한다.
  - `ui/mock` 의 `DashboardRepository` → **현행** `SupabaseDashboardRepository`(임시 Supabase 백엔드)가 실제 화면을 구동.
...
- **화면 이동 방식**: Navigation Compose 골격은 있으나 현재 route는 `main` 하나. 실질 이동은 탭/페이지 상태 + 콜백(`onSectorClick`/`onViolationClick` 등)으로 상위 컨테이너가 페이지를 전환.
...
| 종류 | 존재 여부 | 내용 |
| --- | --- | --- |
| Unit Test | ✅ (최소) | `CartLabelVisibilityTest`(마커 라벨 표시 임계 로직), `ExampleUnitTest`(템플릿), `testutil/MainDispatcherRule`(`Dispatchers.Main` 교체 규칙). |
...
> 과거 존재하던 `MapViewModelTest`, `NextConsecutivePollFailuresTest`는 대상 코드(`MapViewModel`) 제거와 함께 삭제되었다. 테스트 커버리지는 현재 낮으며 확충이 필요하다(10. 기술 부채).
...
- **데이터 레이어 이원화**: `ui/mock`의 Supabase 경로가 실제 화면을 구동하고, `data/`의 Retrofit 계층은 팀 REST BE 대비로 병존. 팀 BE 확정 시 `DashboardRepository` 구현을 교체하고 `data/`로 일원화 필요.
...
- **테스트 커버리지 부족**: 단위 테스트가 최소 수준, UI/E2E 사실상 없음. ViewModel·Repository·매퍼 위주로 확충 필요.
...
- **임시 백엔드 전제**: 현재 Supabase는 인터임(interim). 스키마 변경은 `supabase/*.sql` 마이그레이션으로 관리하되, 일부 테이블에 unique 제약이 없어 `ON CONFLICT` 사용 시 주의(`on conflict do nothing` 무타깃 형태로 처리한 이력 있음).
...
_최종 업데이트 기준 커밋: `bffbfae` (branch `fix/status-row-cart-primary`)_
